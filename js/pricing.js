...
document
  ...
  .addEventListener("click", () => {
    ...
    const accessToken = "your_access_token"; // Вставьте AccessToken клиента
    sendPurchaseRequest("monthly", total, accessToken);
  });
...
document.getElementById("annual-purchase-btn").addEventListener("click", () => {
  ...
  const accessToken = "your_access_token"; // Вставьте AccessToken клиента
  sendPurchaseRequest("annual", total, accessToken);
});
...
document
  ...
  .addEventListener("click", () => {
    ...
    const accessToken = "your_access_token"; // Вставьте AccessToken клиента
    sendPurchaseRequest("onetime", total, accessToken);
  });

// =============================================

function sendPurchaseRequest(plan, total, accessToken) {
  fetch("/api/purchase", {
    method: "POST",
    headers: {
    ...
    .then((data) => {
      alert(`Purchase successful! Redirecting to Monobank...`);
      window.location.href = data.payment_url; // URL для оплаты в Monobank
    })
    .catch((error) => {
